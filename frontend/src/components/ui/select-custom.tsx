...
export function CustomSelect({
  ...
  return (
    <div ref={containerRef} className={cn("relative w-full", isOpen && "z-50", wrapperClassName)}>
      {label && (
        <span className="absolute left-4 top-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/80 pointer-events-none z-10">
          {label}
        </span>
      )}
      <button
      ...
      {isOpen && (
        <div className="absolute left-0 z-50 mt-2 max-h-60 w-full overflow-y-auto rounded-2xl border border-border/80 bg-surface/95 p-1.5 shadow-elevated backdrop-blur-md animate-in fade-in-50 slide-in-from-top-2 duration-200">
          {options.length === 0 ? (
            ...
            options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full text-left rounded-xl px-3 py-2.5 text-xs font-semibold transition-all duration-150 flex items-center justify-between",
                    isSelected
